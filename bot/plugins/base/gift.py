...
# تغییر ۱: ReplyKeyboardRemove اضافه شد
...
async def handle_phone_share(client: Client, message: Message) -> None:
    ...
    # نکته: می‌توانیم همینجا هم ReplyKeyboardRemove بزنیم، اما معمولاً روی پیام متنی نهایی امن‌تر است.
    ...
    try:
        ...
        # تغییر ۲: اضافه کردن ReplyKeyboardRemove به پیام موفقیت
        await message.reply_text(
            ...
            reply_markup=ReplyKeyboardRemove()  # <--- این خط دکمه را پاک می‌کند
        )
